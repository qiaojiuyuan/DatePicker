...
public class DatePickerView extends View {
    ...
    public DatePickerView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,R.styleable.DatePickerView,defStyle,0);
        int n = a.getIndexCount();
        for(int i = 0;i < n;i++) {
            int attr = a.getIndex(i);
            switch (attr) {
                case R.styleable.DatePickerView_datePickerType:
                    dateType = a.getString(attr);
                    break;
            }
        }
        init();
    ...
    private void init() {
        ...
        mPaint.setColor(ContextCompat.getColor(context, R.color.ahlib_color_b2));
        //第二个paint
        nPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        nPaint.setStyle(Style.FILL);
        nPaint.setTextAlign(Align.CENTER);
        nPaint.setColor(ContextCompat.getColor(context, R.color.ahlib_color_b4));
    }
    ...
    private void drawData(Canvas canvas) {
        ...
        canvas.drawText(mDataList.get(mCurrentSelected)+dateType, x, baseline, mPaint);
        // 绘制上方data
    ...
    private void drawOtherText(Canvas canvas, int position, int type) {
        ...
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));
        canvas.drawText(mDataList.get(mCurrentSelected + type * position)+dateType,
