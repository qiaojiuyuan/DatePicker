package com.jeo.datepicker.view;
...
public class CustomDatePicker {
    ...
    private void initDialog() {
        if (datePickerDialog == null) {
            ...
            datePickerDialog.getWindow().setWindowAnimations(R.style.mypopwindow_anim_style);
...
//        minute_text = (TextView) datePickerDialog.findViewById(R.id.minute_text);
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
                String selectTime = sdf.format(selectedCalender.getTime());
    ...
    private void monthChange() {
        ...
        /**修改测试  start*/
        month_pv.setData(month);
        int currentMonth = selectedCalender.get(Calendar.MONTH) + 1;
        if (currentMonth < Integer.parseInt(month.get(0))) {
            //小于最小月份
            selectedCalender.set(Calendar.MONTH, Integer.parseInt(month.get(0)) - 1);
            month_pv.setSelected(0);
            executeAnimator(month_pv);
        } else if (currentMonth > Integer.parseInt(month.get(month.size() - 1))) {
            //大于最大月份
            selectedCalender.set(Calendar.MONTH, Integer.parseInt(month.get(month.size() - 1)) - 1);
            month_pv.setSelected(month.size()-1);
            executeAnimator(month_pv);
        } else {
            //包含在当前中
            selectedCalender.set(Calendar.MONTH, currentMonth - 1);
            int position = 0;
            for (int i = 0;i < month.size();i++) {
                int monthInt = Integer.parseInt(month.get(i));
                if (monthInt == currentMonth) {
                    position = i;
                    break;
                }
            }
            month_pv.setSelected(position);
        }
...
//        int selectedMonth = selectedCalender.get(Calendar.MONTH) + 1;
        int selectedMonth = month_pv.getSelectData();
        ...
        day_pv.setData(day);
        int currentDay = selectedCalender.get(Calendar.DAY_OF_MONTH);
        if (currentDay > Integer.parseInt(day.get(day.size()-1))) {
            //当前选择的日期大于改变后月份的日期，如上个月是31号，现在的月只有30号
            //大于最大天
            selectedCalender.set(Calendar.DAY_OF_MONTH, Integer.parseInt(day.get(day.size()-1)));
            day_pv.setSelected(day.size() - 1);
            executeAnimator(day_pv);
        } else if (currentDay < Integer.parseInt(day.get(0))) {
            //当前选择的天小于改变后的最小天，选择改变后的最小天。如选择的是15日，但改变后的最小的日是20
            //小于最小天
            ...
            for (int i = 0;i < day.size();i++) {
                int dayInt = Integer.parseInt(day.get(i));
                if (dayInt == currentDay) {
                    currentPosition = i;
                    break;
                }
            }
            day_pv.setSelected(currentPosition);
        }
    ...
    private void hourChange() {
        if ((scrollUnits & SCROLL_TYPE.HOUR.value) == SCROLL_TYPE.HOUR.value) {
            ...
            /**修改小时逻辑  start*/
            hour_pv.setData(hour);
            int  currentHour = selectedCalender.get(Calendar.HOUR_OF_DAY);
            if (currentHour < Integer.parseInt(hour.get(0))) {
                //当前小时小于改变后的最小小时
                //小于最小小时
                selectedCalender.set(Calendar.HOUR_OF_DAY, Integer.parseInt(hour.get(0)));
                hour_pv.setSelected(0);
                executeAnimator(hour_pv);
            } else if (currentHour > Integer.parseInt(hour.get(hour.size() - 1))) {
                //当前小时大于改变后的最大小时
                //大于最大小时
                selectedCalender.set(Calendar.HOUR_OF_DAY, Integer.parseInt(hour.get(hour.size()-1)));
                ...
                for (int i = 0;i < hour.size();i++){
                    int hourInt = Integer.parseInt(hour.get(i));
                    if (hourInt == currentHour) {
                        position = i;
                        break;
                    }
                }
                hour_pv.setSelected(position);
            }
        }
    ...
    private void minuteChange() {
        if ((scrollUnits & SCROLL_TYPE.MINUTE.value) == SCROLL_TYPE.MINUTE.value) {
            ...
            /**修改分钟逻辑 start*/
            minute_pv.setData(minute);
            int currentMinute = selectedCalender.get(Calendar.MINUTE);
            if (currentMinute < Integer.parseInt(minute.get(0))) {
                //当前分钟小于改变后的最小分钟
                //小于改变后的最小分钟
                selectedCalender.set(Calendar.MINUTE, Integer.parseInt(minute.get(0)));
                minute_pv.setSelected(0);
                executeAnimator(minute_pv);
            } else if (currentMinute > Integer.parseInt(minute.get(minute.size() - 1))) {
                //当前分钟大于改变后的最大分钟
                //大于改变后的最大分钟
                selectedCalender.set(Calendar.MINUTE, Integer.parseInt(minute.get(minute.size()-1)));
                ...
                for (int i = 0;i < minute.size();i++) {
                    int minuteInt = Integer.parseInt(minute.get(i));
                    if (minuteInt == currentMinute) {
                        currentPosition = i;
                        break;
                    }
                }
                minute_pv.setSelected(currentPosition);
            }
        }
        executeScroll();
    }
...
//                minute_text.setVisibility(View.GONE);
     ...
     * 设置日期控件是否可以循环滚动
     */
    public void setIsLoopExceptYear(boolean isLoop) {
        if (canAccess) {
